...
namespace Mirror
{
    ...
    public class NetworkManager : MonoBehaviour
    {
        ...
        [EditorBrowsable(EditorBrowsableState.Never),
         Obsolete(
             "This is a static property now...use `isHeadless` instead of `IsHeadless()`.  This method will be removed by summer 2019.")]
        ...
        public virtual void OnClientConnect(NetworkConnection conn)
        {
            ...
            {
                Debug.Log("ここでプレイヤーを生成しています");
                // Ready/AddPlayer is usually triggered by a scene load completing. if no scene was loaded, then Ready/AddPlayer it here instead.
                ClientScene.Ready(conn);
                if (autoCreatePlayer)
                {
                    bool addSuccess= ClientScene.AddPlayer();
                    if (ClientScene.localPlayer == null)
                    {
                        Debug.Log("local Player is null");
                    }
                    else if (ClientScene.localPlayer.gameObject == null)
                    {
                        Debug.Log("local Player gameObject is null");
                    }
                    else
                    {
                        Debug.Log(ClientScene.localPlayer.gameObject.name + "がplayer object");
                    }

                    if (addSuccess == false)
                    {
                        Debug.LogWarning("プレイヤー生成に失敗してます");
                    }
                }
            }
        }
        ...
        public virtual void OnClientSceneChanged(NetworkConnection conn)
        {
            ...
            {
                Debug.Log("既にlocalPlayerは居たので気にしない！");
            }
        }
